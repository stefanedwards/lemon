...
set_symmetric_scale <- function(sc, mid) {
  ...
  #     mid = diff(sc$limits)/2
  #   } else {
  #     stop('`mid` cannot be NA if limits are missing.')
  #   }
  # }
  # 
  sc$mid <- mid
...
#' scales for continuous x and y, but ensures that the resulting 
#' 
...
#' df <- do.call(rbind, df)
#' ggplot(df, aes(x=x,y=y)) + geom_point() + facet_grid(a~b, scales='free_x')
